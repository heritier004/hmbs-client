...
useEffect (()=>{
    ...
    const makeReservation = ()=>{
        ...
        Axios.post(`https://movie-bookings-system.herokuapp.com/makeReservation`,article)
        .then((response)=>{
            if(response.status == 200){
                console.log(response.data.code);
         Axios.get(`https://movie-bookings-system.herokuapp.com/pdf/${user[0].id}/${selectedSched}/${response.data.code}`, {responseType: 'arraybuffer'})
        .then((res)=>{
